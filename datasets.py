...
class FiftySaladDataset(Dataset):
    def __init__(self, root_dir, split, split_type):
        super().__init__()
        self.split_type = split_type
        self.act_dict = read_mapping_dict(os.path.join(root_dir, 'mapping_50salad.txt'))

        files = sorted([file for file in os.listdir(os.path.join(root_dir, 'groundtruth')) if file.endswith('.txt')])
        ...
        train_video_list = train_splits[int(split) - 1]
        test_video_list = test_splits[int(split) - 1]
        ...
        self.test_data = {'act_ix_seqs': [], 'dur_seqs': [], 'seq_lens': []}

        self.all_durs = []
        for video_name in train_video_list:
            with open(os.path.join(root_dir, f'groundtruth/{video_name}')) as file_ptr:
                content = file_ptr.read().split('\n')[:-1]
                action_seq, duration_seq = get_label_length_seq(content)
                self.all_durs.extend(duration_seq)
                action_seq_tensor = torch.tensor([self.act_dict[ix] for ix in action_seq], dtype=torch.long)
                duration_seq_tensor = torch.tensor(duration_seq, dtype=torch.float)
                seq_len = len(action_seq)
                for i in range(2, seq_len):
                    self.training_data['obs_act_seqs'].append(one_hot(action_seq_tensor[:i-1], num_classes=19))
                    self.training_data['obs_dur_seqs'].append(duration_seq_tensor[:i-1].unsqueeze(-1))
                    self.training_data['pred_act'].append(action_seq_tensor[i])
                    self.training_data['pred_dur'].append(action_seq_tensor[i])
                    self.training_data['obs_seq_len'].append(len(action_seq_tensor[:i])-1)

        self.dur_std, self.dur_mean = torch.std_mean(torch.tensor(self.all_durs, dtype=torch.float))
        for video_name in test_video_list:
            with open(os.path.join(root_dir, f'groundtruth/{video_name}')) as file_ptr:
                content = file_ptr.read().split('\n')[:-1]
                action_seq, duration_seq = get_label_length_seq(content)
                action_ix_seq = [self.act_dict[ix] for ix in action_seq]
                self.test_data['act_ix_seqs'].append(torch.tensor(action_ix_seq, dtype=torch.long).unsqueeze(-1))
                self.test_data['dur_seqs'].append(torch.tensor(duration_seq, dtype=torch.float32).unsqueeze(-1))
                self.test_data['seq_lens'].append(len(action_seq))

    def __len__(self):
        if self.split_type == 'train':
            return len(self.training_data['obs_act_seqs'])
        else:
            return len(self.test_data['act_ix_seqs'])

    def __getitem__(self, i):
        if self.split_type == 'train':
            return {k: self.training_data[k][i] for k in
                    ['obs_act_seqs', 'obs_dur_seqs', 'pred_act', 'pred_dur', 'obs_seq_len']}
        else:
            return {k: self.test_data[k][i] for k in ['act_ix_seqs', 'dur_seqs', 'seq_lens']}
...
class BreakfastDataset(Dataset):
    def __init__(self, root_dir, split, split_type):
        ...
        self.data = {'act_ix_seqs': [], 'dur_seqs': [], 'seq_lens': []}
        ...
        with open(os.path.join(root_dir, f'{split_type}.split{split}.bundle'), 'r') as file_ptr_0:
            ...
            for video_name in video_list:
                ...
                with open(os.path.join(root_dir, f'groundtruth/{video_name}'), 'r') as file_ptr_1:
                    ...
                    action_seq, duration_seq = get_label_length_seq(content)
                    action_ix_seq = [self.act_dict[ix] for ix in action_seq]
                    self.data['act_ix_seqs'].append(torch.tensor(action_ix_seq, dtype=torch.long).unsqueeze(-1))
        ...
        if self.split_type == 'train':
            # generate n-1 training examples from each training example with n segments
            self.training_data = {'obs_act_seqs': [],
                                  'obs_dur_seqs': [],
                                  'pred_act': [],
                                  'pred_dur': [],
                                  'obs_seq_len': []}
            act_seqs = self.data['act_ix_seqs']
...
def seq_collate_dict(data, time_first=False):
    ...
    batch = {}
    modalities = ['act_ix_seqs', 'dur_seqs']
